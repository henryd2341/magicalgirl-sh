...
async function writeNewGameVariables(
  ...
  // Append optional appearance/background if filled
    ...
    <div class="mg-setup__card">
      ...
      <div v-if="currentStep === 1" class="mg-setup__step">
        ...
        <input
          ...
          placeholder="请输入主角名称"
        />
        <fieldset class="mg-setup__fieldset">
      ...
      <div v-if="currentStep === 2" class="mg-setup__step">
        ...
        <input id="new-game-body-type" v-model="bodyType" class="mg-input" type="text" placeholder="例如：纤细、匀称（留空使用默认值）" />
        <label class="mg-setup__label" for="new-game-hair-color">发色</label>
        <input id="new-game-hair-color" v-model="hairColor" class="mg-input" type="text" placeholder="例如：黑色长发（留空使用默认值）" />
        <label class="mg-setup__label" for="new-game-eye-color">瞳色</label>
        <input id="new-game-eye-color" v-model="eyeColor" class="mg-input" type="text" placeholder="例如：深棕色（留空使用默认值）" />
      </div>
      ...
      <div v-if="currentStep === 3" class="mg-setup__step">
        ...
        <textarea id="new-game-family-bg" v-model="familyBackground" class="mg-input mg-input--textarea" rows="3" placeholder="描述角色的家庭背景（留空使用默认值）"></textarea>
        <label class="mg-setup__label" for="new-game-past-experience">过往经历</label>
        <textarea id="new-game-past-experience" v-model="pastExperience" class="mg-input mg-input--textarea" rows="4" placeholder="描述角色的过往经历（留空使用默认值）"></textarea>
      </div>

      <!-- ═══ Actions ═══ -->
      <div class="mg-setup__actions">
        <button v-if="currentStep > 1" class="mg-btn mg-btn--ghost" type="button" @click="prevStep">
          <i class="fas fa-arrow-left"></i> 上一步
        </button>
        <button v-if="currentStep < totalSteps" class="mg-btn mg-btn--primary" type="button" :disabled="!canProceedFromStep1 && currentStep === 1" @click="nextStep">
          下一步 <i class="fas fa-arrow-right"></i>
        </button>
        <button v-if="currentStep === totalSteps" class="mg-btn mg-btn--primary" type="button" :disabled="!canProceedFromStep1" @click="confirmNewGame">
          <i class="fas fa-check"></i> 确认并开始游戏
        </button>
        <button class="mg-btn mg-btn--ghost" type="button" @click="cancelNewGame">
