...
为支持 provisional message、失败草稿、系统摘要、历史总结、可见性控制与调试审计，聊天消息不再被视为仅包含 `role/content/timestamp` 的简单记录，而应作为具备生命周期与来源信息的领域对象统一建模。
...
interface ChatMessage {
  ...
  source: {
    ...
    tool_call_id?: string
    generated_by: 'user' | 'assistant' | 'engine' | 'summary'
  }
...
`visibility.user_visible`：该消息是否在 UI 中对玩家可见。
`visibility.ai_visible`：该消息是否在后续 Prompt 组装时对 AI 可见。
`lifecycle.provisional`：该消息是否仍处于临时状态，尚未正式提交。
`lifecycle.finalized`：该消息是否已完成最终提交并进入正式历史。
`lifecycle.failed`：该消息是否因工具调用失败、请求中断或校验失败而转为失败草稿。
`source.request_id`：标记该消息所属的 AI 请求，用于将流式文本、工具调用与状态变更绑定到同一请求生命周期。
`source.context_version`：标记该消息生成时所对应的上下文版本，用于防止过期内容污染当前叙事。
`source.tool_call_id`：若该消息由某次工具调用衍生（如系统警告、战斗摘要），则记录关联工具调用标识。
`source.generated_by`：区分消息来源是用户输入、模型输出、引擎生成还是总结模块生成。
`kind`：用于区分普通对话、战斗摘要、系统日志、历史总结和失败草稿等消息类型。

#### 3.3.3 生命周期规则

用户发送的普通输入消息在提交后直接进入正式历史，默认 `provisional=false`、`finalized=true`。
AI 流式输出消息在生成阶段先写入 `provisional buffer`，默认 `provisional=true`、`finalized=false`。
若该轮请求中的工具调用全部校验通过，且关键持久化写入收到 `commitAck`，则该 AI 消息转为正式消息：`provisional=false`、`finalized=true`。
若请求中断、工具调用非法或命令执行失败，则该消息可被：

- 直接丢弃；
- 或保留为失败草稿：`failed=true`、`finalized=false`。

战斗结算摘要、系统警告、历史总结等非自然对话内容由引擎或总结模块生成，角色一般为 `system`，并通过 `kind` 区分用途。

#### 3.3.4 可见性规则

默认情况下，玩家输入与 AI 正常回复同时对用户与 AI 可见。
系统调试消息、错误说明、变量变更摘要可设置为 `user_visible=false` 或 `ai_visible=false`，以控制其仅用于内部审计或恢复流程。
玩家手动隐藏的楼层，其消息记录仍保留在本地数据库中，但后续 Prompt 组装时 `ai_visible=false`。
历史总结消息通常对用户可见、对 AI 可见，但应通过 `kind='history_summary'` 与普通对话隔离，避免混淆

#### 3.3.5 与持久化的关系

`chat_history` 表是 `ChatMessage` 的持久化投影，而非唯一真相源。
数据库结构可在 MVP 阶段适度简化，但领域层必须保留消息生命周期、可见性和来源信息，以支持：

`provisional` → `finalized` 的正式化流程；
...
- **Pass 行动**：将行动让给下一位成员（循环队列）。若当前有闪烁图标，Pass 将**优先消耗非闪烁图标**，否则使一个普通图标变成闪烁图标。
...
1. **可读可写变量（AI 可获取其值，且能通过工具主动修改）**
    ...
    - AI 通过专用领域命令修改。引擎需经过 Zod Schema 校验，确保数值在合法区间（如好感度 0~100），物品增减逻辑也由引擎原子化执行，杜绝非法状态。
...
同时，在 System Prompt 底部追加约束：“你只能通过提供的 (工具名) 工具修改标记为可写的变量。任何尝试修改只读变量的工具调用都将被系统拦截并导致警告。”
...
- 每次用户输入时，上下文对话流获得一个递增的 `context_version`。  
...
为避免 Prompt Builder 在长期扩展后失控，所有注入 AI 上下文的内容必须按来源与优先级进行分类管理。系统将注入源划分为三类：**Hard Injection**、**Retrieval Injection**、**Optional Injection**。

#### 7.4.1 Hard Injection（硬注入）

Hard Injection 指每次请求都必须注入、且优先级最高的内容。这些内容构成 AI 行为边界与当前会话的硬约束，不可因 Token 紧张而随意移除。

典型包括：

- System Prompt（角色定义、工具规则、安全边界、行为约束）
- 当前请求可用的工具清单与工具契约
- 当前 FSM/SessionState 相关约束说明
- 当前请求绑定的必要状态摘要
- 当前场景必须成立的核心设定
- 当前回合不可忽略的系统摘要（如战斗结果摘要）

规则：

- Hard Injection 必须优先占用 Prompt 预算。
- 若预算不足，应优先压缩低优先级注入，而不是移除 Hard Injection。
- Hard Injection 中的内容必须是结构稳定、可审计、可预览的。

#### 7.4.2 Retrieval Injection（检索注入）

Retrieval Injection 指根据当前输入、地点、剧情阶段、关键词命中结果或世界状态动态召回的上下文内容。这类内容用于增强叙事一致性与世界知识覆盖，但不保证每轮都存在。
...
- Retrieval Injection 由检索策略、优先级和 Token 预算共同决定是否注入。
- 同一轮请求中，多个候选条目应按 priority、匹配强度、时效性排序。
- 超出预算的条目应被丢弃，并在调试模式下记录“命中但未注入”的原因。
- Retrieval Injection 的渲染结果只参与本次 Prompt 组装，不回写原始数据库内容。

#### 7.4.3 Optional Injection（可选注入）

Optional Injection 指在预算允许、用户开启相关选项或特定模式下才注入的辅助内容。这类内容通常提升体验、调试能力或沉浸感，但不应影响核心逻辑闭环。
...
- Optional Injection 优先级最低，应最先在预算紧张时被压缩或移除。
- Optional Injection 默认不应影响工具调用边界与核心状态约束。
- 调试类 Optional Injection 在生产环境下默认关闭。

#### 7.4.4 注入优先级原则

Prompt Builder 在组装请求时应遵循以下优先级顺序：
...
当 Token 预算不足时，裁剪顺序应优先从 Optional Injection 开始，再裁剪 Retrieval Injection，最后才考虑压缩非核心状态摘要；Hard Injection 不应被移除。
...
- 各类注入源分别占用的 Token 估算
- 命中的 Retrieval Injection 条目
- 被丢弃的 Retrieval / Optional 条目及原因
- 最终注入顺序
- 最终保留的工具快照与状态摘要版本

这使 Prompt Builder 的行为具备可诊断性，避免“AI 忘设定”却无法定位问题来源。
...
1. **`variable_def`**（变量定义表）  
    ...
    关键字段：`id`（主键，如 `player.hp`）、`category`、`schema`（JSON）、`access_level`、`default_value`、`description`（省略 `created_at`、`updated_at` 等审计字段，下同）
...
4. **`chat_history`**（聊天历史表）  
    作用：持久化存储聊天消息及其生命周期状态，支持 provisional 消息正式化、上下文恢复、历史总结、系统摘要与失败草稿保留。

#### 10.5.4 存档与配置组（3 张表）

1. **`event_log`**（事件日志表）
    作用：持久化事件日志，用于审计、调试、辅助重放
...
由于本系统中的聊天消息不仅承担普通对话存储功能，还承担 provisional 流式输出、系统摘要、失败草稿、历史总结、可见性控制与调试追踪等职责，因此 `chat_history` 不应仅被设计为简单的 `role/content/timestamp` 记录表，而应作为 `ChatMessage` 领域模型的持久化投影。

##### 10.5.x.1 推荐表结构

```sql
CREATE TABLE chat_history (
  id TEXT PRIMARY KEY,                  -- 消息唯一ID（UUID/ULID）
  role TEXT NOT NULL CHECK(role IN ('user', 'assistant', 'system')),
  kind TEXT NOT NULL DEFAULT 'normal',  -- 'normal' | 'battle_summary' | 'system_log' | 'history_summary' | 'failed_draft'
  ...
  tool_call_id TEXT,
  generated_by TEXT NOT NULL DEFAULT 'user', -- 'user' | 'assistant' | 'engine' | 'summary'

  hidden INTEGER NOT NULL DEFAULT 0,         -- 兼容用户手动隐藏语义，可视为 user_visible 的快捷投影
...
- `id`：消息主键，推荐使用 UUID 或 ULID，避免自增 ID 在导入导出、跨存档合并和调试重放场景下带来歧义。
- `role`：消息角色，仅区分 `user` / `assistant` / `system`。
- `kind`：消息类型，用于区分普通消息、战斗摘要、系统日志、历史总结、失败草稿等特殊用途消息。
- `content`：消息正文，存储清洗前或清洗后的策略应由渲染安全层统一规定，但数据库中应始终保存完整原始文本语义。
- `user_visible`：该消息是否在 UI 中默认显示给用户。
- `ai_visible`：该消息是否会在下次 Prompt 组装时注入给模型。
- `provisional`：该消息是否处于临时状态，通常用于流式输出尚未正式提交时。
- `finalized`：该消息是否已完成正式提交，只有正式消息才计入稳定上下文历史。
- `failed`：该消息是否属于失败草稿，用于保留中断生成或非法工具调用后的可恢复文本。
- `request_id`：标记该消息所属的 AI 请求，用于将其与工具调用、状态变更和调试日志关联。
- `context_version`：记录消息生成时对应的上下文版本，用于防止旧请求污染新状态。
- `tool_call_id`：若该消息与某次工具调用强关联（如系统警告、执行失败说明、战斗摘要），则记录对应工具调用 ID。
- `generated_by`：标识消息来源是用户输入、模型输出、引擎生成还是总结模块生成。
- `hidden`：兼容“用户手动隐藏消息”的交互语义；MVP 可与 `user_visible` 联动维护，但领域语义上仍建议保留显式字段。
- `summary_version`：用于标记该消息是否属于某一轮总结结果，或被某一轮总结所覆盖。
- `created_at / updated_at`：用于审计、排序和同步 UI 刷新。
...
- 正常完成的 AI 回复：
  - 流式期间：`provisional = 1`，`finalized = 0`
  - 正式提交后：`provisional = 0`，`finalized = 1`
...
- `provisional = 1` 时，通常应满足 `finalized = 0`
- `failed = 1` 的消息不应同时被视为正式历史
- `ai_visible = 0` 的消息不得参与后续 Prompt 组装
- 被用户手动隐藏的消息默认仍保留在数据库中，不应直接物理删除

##### 10.5.x.4 与消息领域模型的关系

`chat_history` 表是 `ChatMessage` 领域对象的持久化投影，而不是唯一真相源。
领域层仍负责管理：

- provisional → finalized 的状态迁移
- 失败草稿保留与丢弃
- 用户可见 / AI 可见的可见性策略
- 与 request_id / tool_call_id / context_version 的绑定关系
...
若 MVP 阶段希望降低实现复杂度，可采用“领域完整、表结构渐进”的策略：
数据库中至少保留：
...
- `context_version`
- `created_at`
- `tool_call_id`、`generated_by`、`updated_at`、`summary_version` 可在后续版本补齐。

但即便 MVP 做简化，也建议不要省略以下字段：
...
- `finalized`
- `request_id`
因为这些字段直接关系到消息正式化、上下文控制与恢复流程的正确性。

##### 10.5.x.6 索引

为保证消息查询、上下文恢复与调试追踪性能，建立以下索引：

```sql
CREATE INDEX idx_chat_history_created_at ON chat_history(created_at);
CREATE INDEX idx_chat_history_request_id ON chat_history(request_id);
CREATE INDEX idx_chat_history_context_version ON chat_history(context_version);
CREATE INDEX idx_chat_history_ai_visible_created_at ON chat_history(ai_visible, created_at);
```

`created_at` 用于按时间顺序恢复消息历史；
`request_id` 用于定位单次请求产生的全部消息；
`context_version` 用于总结与上下文重建；
`ai_visible` + `created_at` 用于高效筛选可参与 Prompt 组装的消息。

#### 10.5.y 事件日志与检查点快照模型

本系统不采用“完全依赖事件重放恢复状态”的纯事件溯源模式，而采用“事件日志 + 检查点快照 + 持久化快照恢复”的折中方案。

其中：

- `event_log` 负责记录**已成功提交**的领域事件，用于审计、调试、开发者重放与问题追踪；
- `checkpoint_snapshot` 负责记录可直接恢复的状态快照，用于快速回滚、页面刷新恢复、战斗恢复与存档恢复；
- 游戏恢复流程优先依赖最近可用的 `checkpoint_snapshot`，仅在必要时参考 `event_log` 补应用少量后续事件。
...
```sql
CREATE UNIQUE INDEX idx_event_log_event_seq ON event_log(event_seq);
...
CREATE INDEX idx_event_log_command_id ON event_log(command_id);
CREATE INDEX idx_event_log_tool_call_id ON event_log(tool_call_id);
CREATE INDEX idx_event_log_context_version ON event_log(context_version);
```

##### checkpoint_snapshot 推荐表结构
...
```sql
CREATE UNIQUE INDEX idx_checkpoint_snapshot_seq ON checkpoint_snapshot(checkpoint_seq);
CREATE INDEX idx_checkpoint_snapshot_type_created_at ON checkpoint_snapshot(checkpoint_type, created_at);
CREATE INDEX idx_checkpoint_snapshot_related_event_seq ON checkpoint_snapshot(related_event_seq);
CREATE INDEX idx_checkpoint_snapshot_request_id ON checkpoint_snapshot(request_id);
CREATE INDEX idx_checkpoint_snapshot_battle_id ON checkpoint_snapshot(battle_id);
CREATE INDEX idx_checkpoint_snapshot_is_pinned_created_at ON checkpoint_snapshot(is_pinned, created_at);
```

**恢复原则**：

- `event_log` 仅记录已成功提交的事实，不承担唯一恢复真源职责；
- 非法工具调用不记正式 `event_log`
- `checkpoint_snapshot` 必须足以独立恢复到某个稳定状态；
- 页面刷新、战斗恢复、错误回滚优先依赖最近可用检查点；
- MVP 不要求从空状态重放全部事件恢复世界，但应保留事件重放所需的基本追踪字段；
- `snapshot_json` 只保存恢复所需状态，不保存纯视觉或可即时重建的 UI 临时细节。

##### `snapshot_json` 推荐轮廓

`snapshot_json` 是检查点快照的核心恢复载荷，必须足以在不依赖全量事件重放的前提下，将游戏恢复到某个稳定状态。其设计目标是：仅保存恢复所需状态，不保存纯视觉或可即时重建的 UI 临时细节。

推荐结构如下：

```ts
interface CheckpointSnapshotJson {
  snapshot_version: number

  meta: {
    // 用于标识检查点自身及版本信息
    checkpoint_id: string
    checkpoint_type: 'idle_checkpoint' | 'combat_checkpoint' | 'save_checkpoint'
    created_at: string
  ...
  runtime: {
    // 用于恢复 SessionState、PipelineState、上下文版本和状态哈希；
  ...
  request_runtime: {
    // 用于记录活跃请求的最小运行态，不保存不可恢复的网络连接对象；
    active_request: ActiveRequestSnapshot | null
    last_completed_request_id?: string | null
  }

  game_state: {
    // 用于恢复当前游戏会话、变量与进度；
    session: GameSessionSnapshot
    variables: VariableSnapshotRef
    progression: ProgressionSnapshot
  }

  chat_state: {
    // 用于恢复 provisional 消息、待提交系统消息及消息可见性上下文；
    provisional_messages: ProvisionalMessageSnapshot[]
    pending_system_messages: PendingSystemMessageSnapshot[]
    last_visible_message_id?: string | null
  }

  combat_state: {
    // 用于恢复战斗待确认、战斗进行中或刚结束后的局面；
    active_battle: ActiveBattleSnapshot | null
    last_battle_result?: BattleResultSnapshot | null
  }

  recovery: {
    // 用于标记该快照的恢复用途与相关事件锚点。
    rollback_target_event_seq?: number | null
    related_event_seq?: number | null
    restore_reason?: 'request_start' | 'battle_trigger' | 'manual_save' | 'auto_save' | 'error_recovery'
  }
}
```
...
- `snapshot_json` 必须足以独立恢复到某个稳定状态；
- `snapshot_json` 不应保存纯视觉动画状态、滚动条位置、调试抽屉展开状态等非业务必要信息；
- `idle_checkpoint` 和 `combat_checkpoint` 推荐直接嵌入关键变量快照，优先保证恢复正确性；
- `save_checkpoint` 可保存更完整的稳定状态，用于跨刷新与导入导出恢复；
- 页面刷新后不强制恢复中断中的流式网络连接，而是根据 `request_runtime` 决定进入 `ERROR_RECOVERY`、回滚或提示用户重试。
...
- **状态切换**：
  ...
  - 战斗结束进入 `POST_COMBAT_READY` → 战斗 BGM 停止 → 之前暂停的场景 BGM 从悬停点恢复（或淡入重新开始）。。
- **特殊战斗 BGM**：若怪物模板的 `enemy_def` 中指定了 `bgm` asset_id，则战斗优先使用该曲目（如 BOSS 战主题曲）。

### 音频引擎的交互约束

- 在 GENERATING 状态下，不允许 AI 驱动切换 BGM；IN_COMBAT 状态下玩家不能手动切歌。
...
- **训练场**：
  - **自由战斗模式（`training`）**：可使用任何已解锁敌人进行无消耗对战，不影响真实存档，帮助玩家熟悉弱点机制。
  - **挑战模式（`challenge`）** ：固定敌人的高塔挑战，除开战利品（获得的EXP、金钱、物品等）外不影响真实存档。
...
目标：完成统一 JSON 变量系统、关键提交确认机制与聊天/状态正式化的一致性闭环。

- 统一 JSON 变量存储落地：
  - `variable_def`
  - `variable_value`
  - `variable_change_log`
- JSON Schema → 运行时校验器适配
- `update_variables` 白名单与高风险字段限制
- Worker `commitAck` 机制实现
- 聊天消息 provisional → finalized 正式化流程
- 关键命令的事务边界与原子提交
- `chat_history` 与状态写入的一致性保障
- 基础 `schema_version / app_version` 写入
- 基础回滚与持久化失败恢复测试

产出：本地状态写入、聊天正式提交与关键命令持久化具备一致性保障，形成稳定的本地数据闭环。
